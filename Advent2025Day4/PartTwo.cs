...
namespace Advent2025Day4
{
    public static class PartTwo
    {
        ...
        public static int GetTotalRollsThatCanBeMoved(string inputFile)
        {
            int fileNum = 1;
            int totalRolls = 0;
            var outputFile = CreateNewFile(inputFile, fileNum);

            int numCanBeMoved = GetNumberOfRollsThatCanBeMoved(inputFile, outputFile);

            while (numCanBeMoved > 0)
            {
                totalRolls += numCanBeMoved;
                inputFile = outputFile;
                fileNum++;
                outputFile = CreateNewFile(inputFile, fileNum);
                numCanBeMoved = GetNumberOfRollsThatCanBeMoved(inputFile, outputFile);
            }

            return totalRolls;
        }

        private static string CreateNewFile(string inputFile, int fileNum)
        {
            var fileParts = inputFile.Split('\\');
            fileParts[^1] = "outputFile_" + fileNum + ".txt";
            var outputFile = string.Join('\\', fileParts);
        ...
        public static int GetNumberOfRollsThatCanBeMoved(string inputFile, string outputFile)
        {
            ...
            {
                using (var file = File.Open(outputFile, FileMode.OpenOrCreate))
                {
        ...
        private static int CalcNumCanMoveInRow(List<List<char>> rows, int rowIdx)
        {
            var numOccupied = 0;
            var numCanMove = 0;
            List<char> rowToWrite = [.. rows[rowIdx]];
            ...
            var line = string.Join("", rowToWrite);
